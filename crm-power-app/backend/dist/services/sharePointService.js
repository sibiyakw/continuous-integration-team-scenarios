...
class SharePointService {
    ...
    async uploadFile(ticketId, fileName, fileBuffer, mimeType, uploadedBy) {
        try {
            if (!this.siteUrl) {
    ...
    async createSharingLink(fileId, expirationDays = 7) {
        try {
    ...
    async ensureTicketFolderExists(ticketId) {
        try {
            const folderName = `Ticket-${ticketId}`;
    ...
    async getDriveId() {
        ...
        const documentsDrive = drives.value.find((drive) => drive.name.toLowerCase().includes('documents') ||
            drive.name.toLowerCase().includes(this.libraryName?.toLowerCase() || ''));
        return documentsDrive?.id || drives.value[0]?.id;
    }
    getMockFile(fileName, mimeType, uploadedBy) {
        return {
