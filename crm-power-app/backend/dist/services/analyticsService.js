...
const dataverseService_1 = __importDefault(require("./dataverseService"));
class AnalyticsService {
    constructor() {
        this.dataverseService = new dataverseService_1.default();
    }
    ...
    async getAgentPerformance(timeRange = '30d') {
        try {
            const rawData = await this.dataverseService.getTicketMetrics(timeRange);
            if (rawData.agentPerformance) {
                return rawData.agentPerformance.map((agent) => ({
    ...
    async getTicketTrends(timeRange = '90d') {
        try {
            const rawData = await this.dataverseService.getTicketMetrics(timeRange);
            if (rawData.trendData) {
                return rawData.trendData.map((trend) => ({
    ...
    async getTicketResolutionReport(timeRange = '30d') {
        try {
            ...
            tickets.tickets.forEach((ticket) => {
                if (ticket.priority && report.byPriority[ticket.priority]) {
    ...
    async getCustomerSatisfactionReport(timeRange = '30d') {
        try {
            const rawData = await this.dataverseService.getTicketMetrics(timeRange);
            return {
    ...
    calculateResponseTimeDistribution(responseTimeData) {
    ...
    generateMonthlyTicketData(tickets) {
        const monthlyData = {};
